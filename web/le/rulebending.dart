...
class HonestyGame {
  ...
  double _lerp(num min, num max, double t) => min + t * (max - min);

  void _load(String data) {
    final Map<String, Map<String, dynamic>> dataTable = json.decode(data);

    final host = Uri.base.host;
    final lang = (host.indexOf('.') == 2) ? host.substring(0, 2) : 'en';
    final content = dataTable[lang];
    ...
    if (!content.containsKey('questions'))
      throw new StateError('Missing required key: questions');
    if (!(content['questions'] is List<String>))
      throw new StateError('Invalid data: questions');
    if ((content['questions'] as List<String>).isEmpty)
      throw new StateError('Invalid data: no questions');
    if (!content.containsKey('feedback'))
      throw new StateError('Missing required key: feedback');
    if (!content.containsKey('finally'))
      throw new StateError('Missing required key: finally');
    if (!content.containsKey('prefix'))
      throw new StateError('Missing required key: prefix');
    if (!content.containsKey('start-question'))
      throw new StateError('Missing required key: start-question');
    ...
    new Timer(const Duration(seconds: 1),
        () => _toggleScaleRecursive(querySelector('#devil')));

    querySelector('#start-yes-button').onClick.first.then((_) {
      final p = new ParagraphElement();
      final part1 = new SpanElement();
      final part2 = new SpanElement();
      part1.innerHtml = content['start_answer_yes'];
      final blink = content['start_answer_yes_blinking'];
      part2.innerHtml = '&nbsp;$blink';
    ...
    querySelector('#start-no-button').onClick.first.then((_) {
      final p = new ParagraphElement()..innerHtml = content['start_answer_no'];
      _parseStart(p);
    });
    ...
    _questionsPerPage =
        (content.containsKey('per_page')) ? content['per_page'] : 4;
    _questionPool = content['questions'];
    _totalQuestions = (content.containsKey('total'))
        ? content['total']
        : _questionPool.length;

    evilScore = 0;
    _rnd = new Random();
    _currentQuestions = <Question>[];
    ...
    final evilBarContainerRect =
        querySelector('#evilbar-container').getClientRects().first;
    evilBarContainerWidth =
        (evilBarContainerRect.right - evilBarContainerRect.left).toInt();
    ...
    _clickListener = window.onClick.listen((_) {
      ...
      try {
        _currentQuestions.firstWhere((q) => !q.selected);
      } on StateError {
        _pullQuestions(_questionsPerPage);
      }

      final currentBarWidth = ((evilBarContainerWidth ~/ 2) + evilScore);
      _evilBar.style.width = '${currentBarWidth}px';

      final red =
          _lerp(0, 200, currentBarWidth / evilBarContainerWidth).toInt();
  ...
  void _pullQuestions(int num_questions) {
    ...
    if (_questionPool.isEmpty || _drawnQuestions >= _totalQuestions) {
      ...
      while (_feedback.keys.length > 1) {
        ...
        try {
          final found =
              _feedback.keys.firstWhere((key) => (evilScore < int.parse(key)));
      ...
      querySelector('#question-prefix')
        ..innerHtml = _feedback.values.first['title']
        ..style.textAlign = 'center';

      querySelector('#honesty-feedback')
        ..innerHtml = _feedback.values.first['description'];
      ...
      final suggestions = querySelector('#honesty-suggestions');
      if (suggestions != null) {
        final suggestionButton = querySelector('#honesty-suggestions-button');
        final TextAreaElement suggestionTextArea =
            querySelector('#honesty-suggestions-textarea');
        if (suggestionButton != null && suggestionTextArea != null) {
          ...
          suggestionButton.onClick.firstWhere((_) {
            ...
            final thanks = new ParagraphElement()
              ..innerHtml = 'Thank you!'
              ..style.textAlign = 'center';
      ...
      final finalFeedback = querySelector('#honesty-final-feedback')
        ..innerHtml = _finalFeedback;
      ...
      for (var i = 0; i < num_questions; i++) {
        ...
        final question = _questionPool[_rnd.nextInt(_questionPool.length)];

        /// If we roll 9 on d10, negate the score
        final negate = (_rnd.nextInt(10) + 1 == 10);
        var evilPoints = _rnd.nextInt(_questionScoreMax) + 1;
  ...
  void _toggleScaleRecursive(Element e) {
    e.style.transform = (e.style.transform == 'scale(1, 1)')
        ? 'scale(0.8, 0.8)'
        : 'scale(1, 1)';
  ...
  void _toggleBlinkRecursive(Element e) {
    ...
    new Timer(
        const Duration(milliseconds: 500), () => _toggleBlinkRecursive(e));
  }
  ...
  StreamSubscription<MouseEvent> _clickListener;
}
...
class Question {
  Question(String value, DivElement container, this._evilPoints) {
    ...
    _root = new DivElement()..className = 'question';
    _question = new ParagraphElement()..innerHtml = value;
    ...
    _yes.onClick.first.then((_) {
      if (!_selected) {
        ...
        final  halfWidth = evilBarContainerWidth ~/ 2;
    ...
    final alternatives = new DivElement()..className = 'alt';
